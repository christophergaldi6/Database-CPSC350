...
unsigned int Database::assignID(string type)
{
  if (type == "student")
  {
    ...
    do
    {
      temp.id = rand() % 8999999 + 1000000; //random number between 1000000 and 9999999; tried to model ID system used currently at school
      if (++attempts > 1000000) //If this runs more than 1000000 and no available ID is found, program assigns ID of 0.
      {
  ...
  else if(type == "faculty")
  {
    ...
    do
    {
      temp.id = rand() % 999999; //random number between 1 and 9999
      if (++attempts > 1000000) // If this runs more than 10000 times and no free ID has been found, program assigns ID of 0.
      {
        cout << "ID assignment timed out." << endl;
        return 0;
      }
